namespace {{project:namespace}}.Dal 
{
    ...
    /// {{table:comment}}数据操作类
    /// </summary>
    public class {{table:cname}}Dal : MssqlDalBase, I{{table:cname}}Dal
    {
        /// <summary>
        /// 添加一个{{table:comment}}
        /// </summary>
        ...
        public void Add({{table:cname}} {{table:lfcname}}, ICTransaction tran)
        {
            ...
            paramCollection.Add(new DBParam("@{{col:pname}}", cloneObj.{{col:pname}}, {{col:daltype}}{{if:col:string}}, {{col:maxlen}}{{/if:col:string}}));{{/loop:col}}
            ...
            try
            {
                ...
                {{table:lfcname}}.{{pk:col:pname}} = cloneObj.{{pk:col:pname}};
                {{table:lfcname}}.CreateTime = cloneObj.CreateTime;
                {{table:lfcname}}.UpdateTime = cloneObj.UpdateTime;
                {{table:lfcname}}.RVersion = cloneObj.RVersion;
            }
        ...
        public void Add(IList<{{table:cname}}> {{table:lfcname}}List, ICTransaction tran)
        {
            ...
            {
                {{table:lfcname}}List[i].{{pk:col:pname}}  = cloneList[i].{{pk:col:pname}} ;
                {{table:lfcname}}List[i].CreateTime = cloneList[i].CreateTime;
                {{table:lfcname}}List[i].UpdateTime = cloneList[i].UpdateTime;
                {{table:lfcname}}List[i].RVersion = cloneList[i].RVersion;
            }
        }
...
{{/loop:fk}}
        ...
        public void Update({{table:cname}} {{table:lfcname}}, ICTransaction tran)
        {
            ...
            paramCollection.Add(new DBParam("@{{pk:col:pname}}", cloneObj.{{pk:col:pname}}, {{pk:col:daltype}}{{if:col:string}}, {{pk:col:maxlen}}{{/if:col:string}}));{{loop:col|ignparam:pk,CreaterId,CreateTime|}}
            paramCollection.Add(new DBParam("@{{col:pname}}", cloneObj.{{col:pname}}, {{col:daltype}}{{if:col:string}}, {{col:maxlen}}{{/if:col:string}}));{{/loop:col}}
        ...
        public string GetPKSQLCommand({{table:cname}}Searcher {{table:lfcname}}Searcher, out DBParamCollection paramCollection)
        {
            {{table:cname}}Searcher querySearcher = null;
            MssqlQueryParser queryParser = new MssqlQueryParser();
            StringBuilder joinQuery = new StringBuilder();
...
{{/loop:fk}}
        ...
        public {{table:cname}}Dal(MssqlDalFactoryBase dalFactory)
            : base(dalFactory)
        {
